...
/** Renders **bold** spans inside otherwise plain résumé copy. */
...
/* --- Brand marks ------------------------------------------------------- *
 * lucide-react dropped brand icons, so these are inline. All but LinkedIn come
 * from simple-icons; LinkedIn was pulled from that set over trademark,
 * so it uses the standard glyph.                                           */
...
/* Each brand's own colour, tuned per theme. True brand hex fails contrast on
   one background or the other for GitHub (1.11:1 on dark), CodeChef (2.25:1
   on dark) and LeetCode (1.96:1 on light), so those are lightened/darkened
   to stay legible, so these are close-to-brand rather than exact. */
const BRAND: Record<SocialLabel, { light: string; dark: string }> = {
  GitHub: { light: "#181717", dark: "#ffffff" },
  LinkedIn: { light: "#0A66C2", dark: "#3B9AE1" },
  LeetCode: { light: "#C2760A", dark: "#FFA116" },
  CodeChef: { light: "#5B4638", dark: "#C7A98C" },
  Codeforces: { light: "#1B6FA3", dark: "#4DAEE8" },
};

/* Trust-bar marks. Amazon comes from Font Awesome's free brand set (CC BY 4.0)
   since simple-icons dropped it; Handshake from simple-icons. No vector mark
   exists anywhere for Jadavpur University, so callers pass their own
   `fallback` for names with no mark. Each keeps its own viewBox. */
const LOGOS: Record<string, { d: string; viewBox: string }> = {
  Handshake: { d: "M20.728 0 16.49 24h-4.583l1.87-10.532-4.743 3.893L7.856 24H3.272L7.51 0h4.582L9.806 13.012l4.729-3.862L16.145 0h4.583z", viewBox: "0 0 24 24" },
  Amazon: { d: "M257.2 162.7c-48.7 1.8-169.5 15.5-169.5 117.5 0 109.5 138.3 114 183.5 43.2 6.5 10.2 35.4 37.5 45.3 46.8l56.8-56S341 288.9 341 261.4V114.3C341 89 316.5 32 228.7 32 140.7 32 94 87 94 136.3l73.5 6.8c16.3-49.5 54.2-49.5 54.2-49.5 40.7-.1 35.5 29.8 35.5 69.1zm0 86.8c0 80-84.2 68-84.2 17.2 0-47.2 50.5-56.7 84.2-57.8v40.6zm136 163.5c-7.7 10-70 67-174.5 67S34.2 408.5 9.7 379c-6.8-7.7 1-11.3 5.5-8.3C88.5 415.2 203 488.5 387.7 401c7.5-3.7 13.3 2 5.5 12zm39.8 2.2c-6.5 15.8-16 26.8-21.2 31-5.5 4.5-9.5 2.7-6.5-3.8s19.3-46.5 12.7-55c-6.5-8.3-37-4.3-48-3.2-10.8 1-13 2-14-.3-2.3-5.7 21.7-15.5 37.5-17.5 15.7-1.8 41-.8 46 5.7 3.7 5.1 0 27.1-6.5 43.1z", viewBox: "0 0 448 512" },
};
...
export function BrandLogo({
  ...
  name: string;
  className?: string;
  /** Rendered when the name has no mark — explicit, so no stray icons appear. */
  fallback?: ReactNode;
}) {
...
export type SocialLabel = keyof typeof MARKS;
...
export function SocialMark({
  ...
  const key = label as SocialLabel;
  const d = MARKS[key];
  if (!d) return null;
  ...
  return (
    <Mark
      d={d}
      style={
        { "--brand-light": brand.light, "--brand-dark": brand.dark } as CSSProperties
      }
      className={`text-(--brand-light) dark:text-(--brand-dark) ${className ?? ""}`}
    />
  );
}

export function GithubMark({ className }: { className?: string }) {
  return <Mark d={MARKS.GitHub} className={className} />;
}

export function Tag({ children }: { children: ReactNode }) {
  return (
    <span className="rounded-full border border-line bg-surface px-2.5 py-1 font-mono text-[11px] tracking-tight text-muted transition-colors hover:border-line-strong hover:text-fg">
      {children}
    </span>
  );
}
